...
namespace Pool
{
    public class GameObjectsGenerator : MonoBehaviour, IGameObjectsGenerator
    {
        ...
        public Dictionary<EnemyType, List<GameObject>> GameObjectsDictionary { get; private set; }
            ...
            IGameObjectsContainer gameObjectsContainer)
        {
            ...
            GameObjectsDictionary = new Dictionary<EnemyType, List<GameObject>>();
        ...
        private void DestroyAll()
        {
            ...
            foreach (KeyValuePair<EnemyType, List<GameObject>> pair in GameObjectsDictionary)
            {
                if (pair.Value != null)
            ...
            _gameObjectsContainer.Clear();
        }

        private void CreateEnemies(int enemiesCount, BaseEnemyType prefab, EnemyType type)
        {
            if (enemiesCount == 0)
                return;
            ...
            {
                _gameObject = Instantiate(prefab.gameObject, _gameObjectsContainer.GameObject.transform);
                _gameObject.SetActive(false);
                _gameObject.GetComponent<IGameObjectMovement>().Run();
                gameObjectsList.Add(_gameObject);
                _gameObjectsContainer.Add(_gameObject);
            }
        ...
        public bool TryGetObject(EnemyType type, out GameObject result)
        {
            result = GameObjectsDictionary[type].FirstOrDefault(p => p.activeSelf == false);
            return result != null;
        }

        private EnemyType GetGameObjectType()
        {
            ...
            for (int i = 0; i < GameObjectsDictionary.Count(); i++)
            {
                ...
                {
                    _enemyType = GameObjectsDictionary.Keys.ElementAt(i);
                    break;
                }
            }
